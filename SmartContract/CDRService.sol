...
contract CdrContract {
    // Struct to hold hash value of a CDR with the current timestamp
    struct CdrData {
        string hashValue;
        uint256 timestamp;
    }

    // Nested dictionary that stores a dictionary for each producer, that stores their CDRs with their unique Ids as keys
    ...
    function viewCdr(address originator, uint256 uniqueId) public view returns (string memory hashValue, uint256 timestamp) {
